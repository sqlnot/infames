...
async function getStreams(cursor = '') {
    ...
    const url = cursor
        ? `https://api.twitch.tv/helix/streams?game_id=${gameId}&first=100&after=${cursor}&language=es`
        : `https://api.twitch.tv/helix/streams?game_id=${gameId}&first=100&language=es`;
    ...
    try {
        ...
        if (data.data && data.data.length > 0) {
            const filteredStreams = data.data.filter(stream => {
                ...
                return /infames|infamerp|infames roleplay|Infames Legacy|infames/.test(title);
            });
                ...
                if (!currentStreams.has(stream.user_name)) {
                    ...
                    streamSidebarItem.innerHTML = `
                        ...
                        <div class="info">
                            ...
                            <p><span class="viewer-badge"></span>${stream.viewer_count} viewers</p>
                        </div>
                    `;
...
function openStream(username) {
    ...
    iframe.src = `https://player.twitch.tv/?channel=${username}&parent=sqlnot.github.io&allowfullscreen=true&autoplay=true`;
